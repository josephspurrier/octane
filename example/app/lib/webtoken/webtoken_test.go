...
func TestValidJWT(t *testing.T) {
	...
	// Generate a token.
	token := webtoken.New(secret, 999999*time.Hour)
	token.SetClock(mc)
...
func TestInvalidSecret(t *testing.T) {
	...
	// Verify the token.
	token2 := webtoken.New(secret2, 999999*time.Hour)
	token.SetClock(mc)
...
func TestNoSecret(t *testing.T) {
	...
	// Generate a token.
	token := webtoken.New([]byte(""), 999999*time.Hour)
	token.SetClock(mc)
	ss, err := token.Generate("jsmith")
...
func TestFutureJWT(t *testing.T) {
	...
	// Generate a token.
	token := webtoken.New(secret, 24*time.Hour)
	token.SetClock(mc)
	ss, err := token.Generate("jsmith")
	assert.Nil(t, err)
	assert.NotEmpty(t, ss)
...
func TestPastJWT(t *testing.T) {
	...
	// Generate a token.
	token := webtoken.New(secret, 1*time.Minute)
	token.SetClock(mc)
	ss, err := token.Generate("jsmith")
	assert.Nil(t, err)
	assert.NotEmpty(t, ss)
